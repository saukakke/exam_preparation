...
final class SubmitQuestionForReviewAction
{
    ...
    {
        if ($question->author_id !== $actor->id && ! $actor->can('questions.update')) {
            throw ValidationException::withMessages(['question' => 'You are not allowed to submit this question.']);
        }
        if ($question->status !== QuestionStatus::Draft && $question->status !== QuestionStatus::Rejected) {
            throw ValidationException::withMessages(['question' => 'Only draft or rejected questions can be submitted.']);
        }
        return $this->db->transaction(function () use ($actor, $question): Question {
            $question->update(['status' => QuestionStatus::InReview]);
            $question->versions()->create(['version'=>$question->version,'stem'=>$question->stem,'explanation'=>$question->explanation,'snapshot'=>$question->toArray(),'created_by'=>$actor->id]);
